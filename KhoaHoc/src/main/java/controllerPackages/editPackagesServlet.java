...
@WebServlet("/editPackagesServlet")
public class editPackagesServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public editPackagesServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String id = request.getParameter("packagesID");
		Packages packages = new Packages();
		try {
			Connection con = JDBCutils.getConnection();
			packages = PackagesDButils.findPackagesById(id);
		...
		request.setAttribute("updatePackages", packages);
		request.getRequestDispatcher("/packages/updatePackages.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			PackagesDButils.updatePackages(p);
